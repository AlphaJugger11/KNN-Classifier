...
def plot_decision_regions(X, y, clf, res=0.02, cycle_marker=True, legend=1):
    ...
    # Ensure Z is numeric
    if isinstance(Z[0], (np.bytes_, str)):
        unique_classes = list(np.unique(Z))
        Z = np.array([unique_classes.index(label) for label in Z])

    # Convert Z to a valid numerical type
    ...
    for idx, cl in enumerate(np.unique(y)):
        plt.scatter(x=X[y == cl, 0], y=X[y == cl, 1],
                    ...
                    marker=next(marker_gen), label=cl)
